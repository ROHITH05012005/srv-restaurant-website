...
def main():
    ...
    print(f"[START] Fast sequential mode downloader for {total} images (no API key, Flux default)...")
    ...
    for i, (filename, prompt) in enumerate(menu_images.items(), 1):
        ...
        # We OMIT model parameter to fall back to the default Flux model (which is fast)
        ...
        while retries > 0 and not success:
            try:
                ...
                success = True
                downloaded += 1
                # Small pause to clear server queue
                time.sleep(1.5)
            except Exception as e:
                retries -= 1
                if "402" in str(e):
                    # Queue full error, wait a bit longer to let queue clear
                    wait_time = 7.0 - retries
                    if retries == 0:
                        print(f" FAILED! ({e})")
                        failed += 1
                    else:
                        print(f" (Queue full, waiting {wait_time:.1f}s)...", end="", flush=True)
