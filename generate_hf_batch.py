...
if not hf_token:
    ...
    print("Please create a .env file with HF_TOKEN=your_token or export the variable.")
    sys.exit(1)
...
for category in menu_data:
    for item in category.get('items', []):
        if item['image'] == "logo.png":
            item_name = item['name']
            print(f"\nGenerating AI image for: {item_name} using HF...")
            try:
                # Build Prompt
                prompt = f"Mouth-watering award-winning professional food photography of {item_name}. A premium, close-up shot of the freshly cooked dish on a traditional dark slate plate, served with fresh garnishes of coriander, side condiments in elegant ceramic bowls. Dramatic warm side-lighting, steam gently rising, depth of field, high-end culinary presentation, 8k resolution, shot with Sony A7R V, highly detailed and photorealistic."
