...
items_to_fix = ["Vanilla", "Strawberry", "Butterscotch", "Chocolate", "Pista"]
updated = 0

for category in menu_data:
    if category.get('category') == 'Ice Cream':
        for item in category.get('items', []):
            if item['name'] in items_to_fix:
                new_name = item['name'] + " Ice Cream"
                print(f"Fixing {item['name']} -> {new_name}")
                item['name'] = new_name
                
                # Regenerate image
                try:
                    prompt = f"{new_name} authentic delicious dessert, premium high-end luxury restaurant photography, dark moody background, golden lighting, highly detailed, 4k resolution, professional food photography"
...
if updated > 0:
    ...
    subprocess.run(['git', 'commit', '-m', 'Fix ice cream names and regenerate images'], check=True)
