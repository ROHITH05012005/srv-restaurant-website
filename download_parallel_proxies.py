...
menu_images = {
    ...
    # EXTRA WEB ASSETS (To remove Unsplash dependencies)
    "hero_bg_premium.png": "A premium, wide-angle luxury South Indian restaurant dining ambiance background, warm candlelight lighting, elegant setup, dramatic bokeh, high-end photography.",
    "about_main_premium.png": "A premium, realistic photo of an elegant modern restaurant interior, with tables set with authentic South Indian dishes, warm inviting atmosphere, professional architectural food photography.",
    "about_sec_premium.png": "A premium, realistic close-up photography of authentic South Indian meals, including crispy dosa, idli, vada, and sambar on a traditional banana leaf, dramatic lighting, steam rising.",
    "dosa_special_premium.png": "A premium, realistic close-up photography of a large, golden-brown, paper-thin crispy roast dosa folded into a cone, served with multiple chutneys and hot sambar in small bowls on a dark plate. Food photography.",
    "filter_coffee_premium.png": "A premium, realistic close-up photography of traditional South Indian filter coffee in a brass tumbler and davara, with thick froth on top, coffee being poured from height creating a beautiful stream. Dramatic lighting.",
    "idli_premium.png": "A premium, realistic close-up photography of soft, fluffy steaming hot white idlis served with coconut chutney and spicy sambar on a dark ceramic platter. Food photography.",
    "gallery_snacks_premium.png": "A premium, realistic close-up photography of a plate of Indian tea-time snacks, including crispy hot samosa, medu vada, and pakora, served with mint and tamarind chutney. Food photography.",
    "avatar_ramesh_premium.png": "A clean close-up studio portrait of a friendly-looking middle-aged Indian man, smiling, warm eyes, professional headshot.",
    "avatar_priya_premium.png": "A clean close-up studio portrait of a friendly-looking young Indian woman, smiling, warm eyes, professional headshot.",
    "avatar_suresh_premium.png": "A clean close-up studio portrait of an elderly, friendly-looking Indian man with gray hair, smiling, warm eyes, professional headshot.",
}
...
def load_proxies():
    global proxies
    print("[PROXIES] Fetching fresh proxy list...")
    req = urllib.request.Request(PROXY_URL, headers={'User-Agent': 'Mozilla/5.0'})
    try:
        data = urllib.request.urlopen(req, timeout=15).read().decode('utf-8')
        p_list = [p.strip() for p in data.split('\n') if p.strip()]
        with lock:
            proxies = p_list
        print(f"[PROXIES] Loaded {len(proxies)} fresh proxies.")
    except Exception as e:
        print(f"[PROXIES] Error fetching proxies: {e}")

def get_proxy():
    global proxies
    with lock:
        if len(proxies) < 15:
            # Run loading in a background fetch or wait
            pass
        if not proxies:
            return None
...
def download_one_image(filename, prompt, index, total):
    ...
    while retries > 0:
        proxy = get_proxy()
        if not proxy:
            # If no proxies left, reload
            load_proxies()
            proxy = get_proxy()
            if not proxy:
                print(f"[{index}/{total}] No proxies available! Waiting 5s...")
                time.sleep(5)
                retries -= 1
                continue
        ...
        try:
            ...
            if len(content) > 1000:
                with open(filename, "wb") as f:
                    f.write(content)
                print(f"[{index}/{total}] Done: {filename} (via {proxy} in {elapsed:.1f}s)")
                # Put the working proxy back at the end of the queue
                return_proxy(proxy)
                return True
            else:
                # Returned an error page or short response
                retries -= 1
                print(f"[{index}/{total}] Small response ({len(content)} bytes) via {proxy}. Retrying...")
        except Exception as e:
            elapsed = time.time() - start
            retries -= 1
            # Proxy failed, discard it (do not return it to list)
            # print(f"[{index}/{total}] Proxy failed ({proxy}): {e} in {elapsed:.1f}s")
            pass

    print(f"[{index}/{total}] FAILED completely: {filename}")
    return False
...
def main():
    ...
    load_proxies()
    if not proxies:
        print("[ERROR] Failed to load any proxies. Exiting.")
        return

    # Count actual missing files
    missing_tasks = []
    for i, (filename, prompt) in enumerate(menu_images.items(), 1):
        if not (os.path.exists(filename) and os.path.getsize(filename) > 0):
            missing_tasks.append((filename, prompt, i, len(menu_images)))

    total_missing = len(missing_tasks)
    print(f"[START] Starting parallel proxy downloader for {total_missing} missing images...")

    if total_missing == 0:
        print("[INFO] All images already exist!")
        return

    # We use a ThreadPoolExecutor with 8 parallel workers
    with ThreadPoolExecutor(max_workers=8) as executor:
        futures = {
            executor.submit(download_one_image, filename, prompt, i, total): filename
            for filename, prompt, i, total in missing_tasks
        }
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                print(f"[ERROR] Task generated an exception for {fname}: {exc}")

    elapsed_all = time.time() - start_time
    print(f"\n[DONE] Successfully downloaded {success_count}/{total_missing} missing images in {elapsed_all:.1f}s!")
