...
def download_image(args):
    index, total, filename, prompt, headers = args
    ...
    while retries > 0:
        try:
            ...
            if retries == 0:
                with print_lock:
                    print(f"[{index}/{total}] FAILED: {filename} | Error: {e}")
                return ("failed", filename)
            else:
                with print_lock:
                    print(f"[{index}/{total}] Retrying: {filename} ({e})")
                time.sleep(4.0)
...
def main():
    ...
    if api_key:
        ...
        print(f"[OK] Using API Key - Parallel mode with 4 threads!")
    else:
        print("[WARN] No API key - anonymous mode.")

    total = len(menu_images)
    print(f"[START] Parallel downloader for {total} images (4 threads)...\n")

    tasks = [
        (i, total, filename, prompt, headers)
